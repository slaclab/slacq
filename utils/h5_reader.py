...
def load_signal_data(group):
    ...
    for signal_name, time in SIGNAL_TIME_MAP.items():
        if (
            signal_name not in group
        ):  # if an event is missing some signals like the very first ones in 2022 (they are missing the decay reference), skip the missing items
            continue
        ...
        if x is None:
            x = np.arange(
                y.shape[0]
            )  # if no time is available, create an x-axis starts from 0 to the length of y
...
def load_event_data_for_classification(path, event_path):
    ...
    with h5py.File(path, "r") as f:
        ...
        saved_q_loaded = get_scalar(
            group, SAVED_Q_LOADED_KEYS
        )  # read the saved loaded Q
